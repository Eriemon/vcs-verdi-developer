...
VCS Verdi Developer 用来帮助 AI 编程代理更可靠地处理 Synopsys VCS 与 Verdi 工作流。仓库中包含工作流说明、可复用夹具、参考文档、评测样例，以及面向编译、仿真、FSDB 生成、RC 布局、Verdi 交互和远程验证的确定性辅助脚本。
...
| `scripts/` | 环境检查、RC 生成、smoke 规划与远程 wrapper 的确定性脚本。 |
| `references/` | VCS/Verdi 流程规则、RC 格式说明、交互说明和验证门禁。 |
| `assets/` | 最小夹具与波形布局模板。 |
| `evals/` | 定义 with-skill 预期行为的评测样例。 |
| `RELEASE_RECEIPT.json` | 导入的 `v0.1.0` 发布包来源记录。 |
...
2. 用 `scripts/generate_rc.py` 生成或复核波形布局 RC。
3. 用 `scripts/smoke_vcs_verdi.py --dry-run` 规划最小 VCS/Verdi smoke 流程。
4. 只有在命令计划和环境结论都被确认后，才执行真实流程。
