...
- `scripts/coverage_flow.py`: plan or execute VCS `-cm` coverage and URG report checks, including conditional `VCS_USE_MALLOC=1` retry after the known internal URG crash shape.
...
- `scripts/urg_coverage_matrix.py`: run URG wrapper/direct `urg1` diagnostic variants for line, cond, tgl, and line+cond+tgl coverage reports, with the same conditional `VCS_USE_MALLOC=1` retry for the declared fallback path.
- `scripts/urg_troubleshoot.py`: run the remote-host URG troubleshooting loop for vendor wrapper versus overlay wrapper, `VCS_ARCH_OVERRIDE=linux`, loader variants, metric variants, and bash normalization probes.
...
- `scripts/autoverifix_vcs_flow.py`: plan and optionally execute AutoVeriFix-style VCS compile, `simv`, URG coverage, scoreboard log parsing, and non-GUI `fsdbreport`/`verdi -ssf` checks.
- `scripts/aiss_vcs_flow.py`: plan AISS-style VCS/VPD simulation and guard unsupported or incomplete gate-level netlist targets.
