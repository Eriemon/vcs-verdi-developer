...
1. Identify the task type: non-GUI VCS simulation, VHDL compile, DPI shared-library simulation, FSDB/VCD dump generation, Verdi RC layout, guarded Verdi interactive control, or ThreadFPGA remote validation.
...
6. For ThreadFPGA, follow `references/threadfpga-validation.md` plus the strict gate in `references/threadfpga-gate.md`, and use the `erie-remote-ssh` skill. Upload only explicit fixture or validation bundles, never the whole local skill-development tree.
...
8. Before reporting readiness, apply `references/review-checklist.md` and run `scripts/run_quality_gate.py --json` locally. Report `local_confidence` separately from `eda_execution_confidence`; only fresh ThreadFPGA or equivalent EDA evidence can make EDA execution confidence pass.
...
- `scripts/remote_threadfpga_gate.py`: plan the minimal ThreadFPGA bundle and validate detached evidence.
