...
MALLOC_RETRY_REASONS = {
    "urg_internal_ucapi_snpsmalloc_failure",
    "urg_internal_non_ucapi_failure",
    "urg_internal_stack_trace_ucapi_patch_not_applicable",
}
    ...
    for metrics in METRIC_SETS:
        ...
        for entry in ENTRIES:
            for full64_mode in FULL64_MODES:
                ...
                exe = str(vcs_home / "bin" / "urg") if entry == "urg" else str(vcs_home / "linux64" / "bin" / "urg1")
                cmd = [exe]
...
def _run_variant(variant: dict, *, workdir: Path, timeout: int, base_env: dict[str, str], ucapi_scan: dict | None) -> dict:
    ...
    result = run_once(env)
    if env.get("VCS_USE_MALLOC") == "1" or result.get("status") != "failed" or result.get("reason") not in MALLOC_RETRY_REASONS:
        return result
    retry_env = env.copy()
    retry_env["VCS_USE_MALLOC"] = "1"
    retry = run_once(retry_env)
